...
from utils.constants import Q_X
...
class ContentQueryChecker:
    ...
    def __call__(self, func):
        ...
        new_params = [
            ...
            Parameter('fields', Parameter.KEYWORD_ONLY, annotation=Optional[List[str]],
                      default=Query(None, regex=f'({q_str})$')),
            Parameter('q', Parameter.KEYWORD_ONLY, annotation=Optional[List[str]],
                      default=Query(None, regex=Q_X.format(cols=f'({q_str})') if q_str else '^[\w]+$|^[\w]+:[\w]+$')),
            Parameter('sort', Parameter.KEYWORD_ONLY, annotation=Optional[List[str]],
                      default=Query(None, explode=True)),
        ]
        ...
        if self._cols:
            for item in self._cols:
                ...
                if col_name == 'password':  # Skip password field if present
                    ...
                    new_params.append(
                        Parameter(
                            f"{col_name}_range_min",
                            Parameter.KEYWORD_ONLY,
                            annotation=Optional[str],
                            default=Query(None, description=f"Filter {col_name} from (e.g., '2023-01-01')")
                        )
                    )
                    new_params.append(
                        Parameter(
                            f"{col_name}_range_max",
                            Parameter.KEYWORD_ONLY,
                            annotation=Optional[str],
                            default=Query(None, description=f"Filter {col_name} to (e.g., '2023-01-20')")
                        )
                    )
                    ...
                    new_params.append(
                        Parameter(
                            ...
                            Parameter.KEYWORD_ONLY,
                            annotation=Optional[str],
                            default=Query(None, description=f"Filter {col_name} from.")
                        )
                    )
                    new_params.append(
                        Parameter(
                            ...
                            Parameter.KEYWORD_ONLY,
                            annotation=Optional[str],
                            default=Query(None, description=f"Filter {col_name} to.")
                        )
                    )
                else:
                    new_params.append(
